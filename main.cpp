...
typedef struct {
	...
	int missingPart; // the empty part of the hexagon for a player to pass through
} hexagon_t;
...
float maxScale; // after this scale value, the scale of the hexagons will be initialized to initialScale
float initialScale; // initial scale
...
void initializeGlobals()
{
	...
	for (int i = 0; i < 4; i++)
	{
		hexagons[i].color = { rand() % 100 / 300.0f + 0.7f, rand() % 100 / 300.0f + 0.7f,rand() % 100 / 300.0f + 0.7f };
		hexagons[i].scale = hexagonScales[i];
		hexagons[i].missingPart = rand() % 6;
	}
	background = { rand() % 100 / 200.0f, rand() % 100 / 200.0f ,rand() % 100 / 200.0f };
}

// to draw circle, center at (x,y) and radius r
...
void onTimer(int v) {
	...
	if (game.isStarted && !game.pause)
	{
		...
		if (timerCount % 25 == 0 && COLOR_CHANGE)
		{
			for (int i = 0; i < 4; i++)
			{
				hexagons[i].color = { rand() % 100 / 200.0f + 0.2f, rand() % 100 / 200.0f + 0.2f,rand() % 100 / 200.0f + 0.2f };
			}
